...
export function TryGetGoodPrefixDate(sparkline: $WaybackSparkLineObject) { // retard compiler hacks
    let best = {
        year: 'none',
        month: 'none',
        month_captures: 0
    ...
    for (let i = keys.length - 1; i > 0; i--) { // get most recent, to contrast failing on oldest
        let year_key = keys[i];
...
export async function TryFindGoodCaptureURL(url: string, date_prefix: string) {
...
export async function BulkDownloader(wb_db_int: WaybackDatabaseInterface, interval = 1000, retry = true) { // PERHAPS EVENTUALLY MERGE TimeMap with Progress, just append the bool and status code...
    let urllist = await wb_db_int.GetURLList(); // i think we can derive urllist from timemap?
